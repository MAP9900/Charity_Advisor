"""
Build a cleaned IRS charity dataset by merging multiple EO BMF CSV extracts.

Usage:
    python scripts/build_clean_irs_dataframe.py
"""

from __future__ import annotations
from pathlib import Path
from typing import Dict, List
import pandas as pd

REGION_FILES: List[str] = [
    "data/eo1.csv", #Includes Connecticut, Maine, Massachusetts, New Hampshire, New Jersey, New York, Rhode Island, Vermont
    "data/eo2.csv", #Includes Delaware, District of Columbia, Illinois, Indiana, Iowa, Kentucky, Maryland, Michigan, Minnesota,
     # Nebraska, North Carolina, North Dakota, Ohio, Pennsylvania, South Carolina, South Dakota, Virginia, West Virginia, Wisconsin.
    "data/eo3.csv", #Includes Alabama, Alaska, Arkansas, Arizona, California, Colorado, Florida, Georgia, Hawaii, Idaho, 
    # Kansas, Louisiana, Mississippi, Missouri, Montana, Nevada, New Mexico, Oklahoma, Oregon, Texas, Tennessee, Utah, Washington, Wyoming
    "data/eo4.csv",] #Includes International and all others.

OUTPUT_FILE = Path("data/cleaned_irs_charities.csv")
#Simple patch to filter NTEE Codes. 
GENERIC_TO_NTEE: Dict[str, List[str]] = {
    "A0": ["A50", "A51", "A52", "A54", "A56", "A57", "A80"],
    "A1": ["A60", "A61", "A62", "A63", "A65", "A68", "A69", "A6A", "A6B", "A6C", "A6E"],
    "A2": ["A30", "A31", "A32", "A33", "A34"],
    "A3": ["A20", "A23", "A25", "A40", "A70", "A84", "A99"],
    "A4": ["A01", "A02", "A03", "A05", "A11", "A12", "A19", "A26", "A90"],
    "B0": ["B20", "B21", "B24", "B25", "B28"],
    "B1": ["B30", "B40", "B41", "B42", "B43", "B50", "B60"],
    "B2": ["B70", "B80", "B82", "B83", "B84"],
    "B3": ["B90", "B92", "B94", "B99"],
    "B4": ["B01", "B02", "B03", "B05", "B11", "B12", "B19"],
    "X0": ["X20", "X21", "X22", "X30", "X40", "X50", "X70"],
    "X1": ["X80", "X81", "X82", "X83", "X84"],
    "X2": ["X90", "X99"],
    "X3": ["X01", "X02", "X03", "X05", "X11", "X12"],
    "E0": ["E20", "E21", "E22", "E24"],
    "E1": ["E30", "E31", "E32", "E40", "E42"],
    "E2": ["E90", "E91", "E92"],
    "E3": ["E50", "E60", "E61", "E62", "E65", "E70", "E80", "E86", "E99"],
    "E4": ["E01", "E02", "E03", "E05", "E11", "E12", "E19"],
    "F0": ["F20", "F21", "F22", "F50", "F52", "F53", "F54"],
    "F1": ["F30", "F31", "F32", "F33", "F70", "F80", "F99"],
    "F2": ["F40", "F42", "F60"],
    "F3": ["F01", "F02", "F03", "F05", "F11", "F12", "F19"],
    "G0": ["G40", "G41", "G42", "G43", "G44", "G45", "G48", "G50", "G51", "G54", "G60", "G61", "G70"],
    "G1": ["G20", "G25", "G30", "G80", "G81", "G83", "G84"],
    "G2": ["G90", "G92", "G94", "G96", "G98", "G9B", "G99"],
    "G3": ["G11", "G12", "G19"],
    "G4": ["G01", "G02", "G03", "G05"],
    "H0": ["H20", "H25", "H30", "H80", "H81", "H83", "H84"],
    "H1": ["H40", "H41", "H42", "H43", "H44", "H45", "H48", "H50", "H51", "H54", "H60", "H61", "H70"],
    "H2": ["H90", "H92", "H94", "H96", "H98", "H9B", "H99"],
    "H3": ["H01", "H02", "H03", "H05", "H11", "H12", "H19"],
    "C0": ["C20", "C27", "C35"],
    "C1": ["C30", "C32", "C34", "C36"],
    "C2": ["C40", "C41", "C42", "C50", "C60", "C99"],
    "C3": ["C01", "C02", "C03", "C05", "C11", "C12", "C19"],
    "D0": ["D20", "D40", "D60", "D61"],
    "D1": ["D30", "D31", "D32", "D33", "D34"],
    "D2": ["D50", "D99"],
    "D3": ["D01", "D02", "D03", "D05", "D11", "D12", "D19"],
    "I0": ["I20", "I21", "I23", "I70", "I71", "I72", "I73"],
    "I1": ["I30", "I31", "I40", "I43", "I44"],
    "I2": ["I50", "I51", "I60"],
    "I3": ["I80", "I83", "I99"],
    "I4": ["I01", "I02", "I03", "I05", "I11", "I12", "I19"],
    "J0": ["J20", "J21", "J22", "J99"],
    "J1": ["J30", "J32", "J33"],
    "J2": ["J03", "J40"],
    "J3": ["J01", "J02", "J05", "J11", "J12", "J19"],
    "K0": ["K30", "K31", "K34", "K35", "K36"],
    "K1": ["K20", "K25", "K26", "K28"],
    "K2": ["K40", "K50", "K99"],
    "K3": ["K01", "K02", "K03", "K05", "K11", "K12", "K19"],
    "L0": ["L20", "L21", "L22", "L25"],
    "L1": ["L40", "L41"],
    "L2": ["L30", "L50", "L80", "L81", "L82", "L99"],
    "L3": ["L01", "L02", "L03", "L05", "L11", "L12", "L19"],
    "M0": ["M20", "M23", "M24", "M99"],
    "M1": ["M40", "M41", "M42"],
    "M2": ["M01", "M02", "M03", "M05", "M11", "M12", "M19"],
    "N0": ["N20", "N30", "N31", "N32"],
    "N1": ["N50", "N60", "N61", "N62", "N63", "N64", "N65", "N66", "N67", "N68", "N69", "N6A"],
    "N2": ["N70", "N71", "N72"],
    "N3": ["N01", "N02", "N03", "N05", "N11", "N12", "N19"],
    "O0": ["O20", "O21", "O22", "O23"],
    "O1": ["O30", "O31", "O40", "O41", "O42", "O43"],
    "O2": ["O50", "O51", "O52", "O53", "O54", "O55", "O99"],
    "O3": ["O01", "O02", "O03", "O05", "O11", "O12", "O19"],
    "P0": ["P30", "P31", "P32", "P33", "P40", "P42", "P43", "P44", "P45", "P46"],
    "P1": ["P80", "P81", "P82", "P84", "P85", "P86", "P87"],
    "P2": ["P58", "P60", "P61", "P62"],
    "P3": ["P70", "P72", "P73", "P74", "P75"],
    "P4": ["P20", "P21", "P22", "P24", "P26", "P27", "P28", "P29", "P50", "P51", "P52", "P99"],
    "P5": ["P01", "P02", "P03", "P05", "P11", "P12", "P19"],
    "Q0": ["Q30", "Q31", "Q32", "Q33", "Q70", "Q71"],
    "Q1": ["Q20", "Q21", "Q22", "Q23"],
    "Q2": ["Q40", "Q41", "Q42", "Q43", "Q99"],
    "Q3": ["Q01", "Q02", "Q03", "Q05", "Q11", "Q12", "Q19"],
    "R0": ["R20", "R22", "R23", "R24", "R25", "R26", "R30"],
    "R1": ["R40", "R60", "R61", "R62", "R63", "R67", "R99"],
    "R2": ["R01", "R02", "R03", "R05", "R11", "R12", "R19"],
    "S0": ["S20", "S21", "S22"],
    "S1": ["S30", "S31", "S32"],
    "S2": ["S40", "S41", "S43", "S46", "S47", "S50", "S80", "S81", "S82", "S99"],
    "S3": ["S01", "S02", "S03", "S05", "S11", "S12", "S19"],
    "T0": ["T20", "T21", "T22", "T23", "T90"],
    "T1": ["T30", "T31", "T70"],
    "T2": ["T40", "T50", "T99"],
    "T3": ["T01", "T02", "T03", "T05", "T11", "T12", "T19"],
    "U0": ["U30", "U31", "U33", "U34", "U36"],
    "U1": ["U40", "U41", "U42"],
    "U2": ["U20", "U21", "U50", "U99"],
    "U3": ["U01", "U02", "U03", "U05", "U11", "U12", "U19"],
    "V0": ["V20", "V21", "V22", "V23", "V24", "V25", "V26"],
    "V1": ["V30", "V31", "V32", "V33", "V34", "V35", "V36", "V37", "V99"],
    "V2": ["V01", "V02", "V03", "V05", "V11", "V12", "V19"],
    "W0": ["W30", "W40", "W50", "W60", "W99"],
    "W1": ["W70", "W71", "W72", "W73"],
    "W2": ["W01", "W02", "W03", "W05", "W11", "W12", "W19"],
    "Y0": ["Y30", "Y33", "Y34", "Y40"],
    "Y1": ["Y20", "Y22", "Y99"],
    "Y2": ["Y50", "Y60", "Y70"],
    "Y3": ["Y01", "Y02", "Y03", "Y05", "Y11", "Y12", "Y19"],}

ALLOWED_NTEE_CODES = {
    code
    for codes in GENERIC_TO_NTEE.values()
    for code in codes}

def load_raw_irs_data(region_files: List[str]) -> pd.DataFrame:
    """Load and concatenate the raw IRS EO BMF CSVs."""
    frames = []
    for file_path in region_files:
        path = Path(file_path)
        if not path.exists():
            print(f"[warn] File not found: {path}")
            continue
        print(f"Loading {path} ...")
        df = pd.read_csv(path, dtype=str, low_memory=False)
        df["source_file"] = path.name
        frames.append(df)

    if not frames:
        raise FileNotFoundError("No IRS CSV files were found. Update REGION_FILES with correct paths.")
    merged = pd.concat(frames, ignore_index=True)
    print(f"Loaded {len(frames)} files; combined rows: {len(merged):,}")
    return merged

def normalize_columns(df: pd.DataFrame) -> pd.DataFrame:
    """Normalize column names and keep only the canonical set we care about."""
    df = df.copy()
    df.columns = df.columns.str.strip().str.lower()

    column_map: Dict[str, str] = {
        "ein": "ein",
        "name": "name",
        "city": "city",
        "state": "state",
        "subseccd": "subseccd",
        "subsection": "subsection",
        "ntee_cd": "ntee_code",
        "nteecc": "ntee_code",
        "ntee_code": "ntee_code",}
    renamed = {}
    for col in df.columns:
        target = column_map.get(col, col)
        renamed[col] = target
    df = df.rename(columns=renamed)

    wanted_cols = ["ein", "name", "city", "state", "subseccd", "subsection", "ntee_code", "source_file"]
    for col in wanted_cols:
        if col not in df.columns:
            df[col] = pd.NA

    return df[wanted_cols]

def clean_irs_dataframe(df: pd.DataFrame) -> pd.DataFrame:
    """Clean and filter the IRS DataFrame down to 501(c)(3) charities with valid NTEE codes."""
    df = df.copy()
    total_before = len(df)
    print(f"Starting rows: {total_before:,}")

    df = df.dropna(subset=["ein", "name"])
    print(f"Rows after dropping missing EIN/NAME: {len(df):,}")

    df["ein"] = (
        df["ein"]
        .astype(str)
        .str.replace(r"[^0-9]", "", regex=True)
        .str.zfill(9))
    df = df[df["ein"].str.len() == 9]
    print(f"Rows after EIN normalization: {len(df):,}")

    df["state"] = df["state"].astype(str).str.strip().str.upper()
    df = df[df["state"].str.match(r"^[A-Z]{2}$", na=False)]
    print(f"Rows after state filtering: {len(df):,}")

    df = df[~df["ntee_code"].isna()]
    df["ntee_code"] = df["ntee_code"].astype(str).str.strip().str.upper()
    df = df[df["ntee_code"].isin(ALLOWED_NTEE_CODES)]
    print(f"Rows after restricting to allowed NTEE codes: {len(df):,}")

    df["ntee_major"] = df["ntee_code"].str[0]
    df = df[df["ntee_major"].str.match(r"^[A-Z]$", na=False)]
    print(f"Rows after ntee_major derivation: {len(df):,}")

    def is_501c3(row: pd.Series) -> bool:
        subseccd_raw = row.get("subseccd")
        subsection_raw = row.get("subsection")
        subseccd = "" if pd.isna(subseccd_raw) else str(subseccd_raw).strip()
        subsection = "" if pd.isna(subsection_raw) else str(subsection_raw).strip().lower()
        if subseccd.isdigit():
            return subseccd == "3"
        if subsection:
            return subsection == "3" or subsection.endswith("03") or subsection.startswith("3")
        return False

    df = df[df.apply(is_501c3, axis=1)]
    print(f"Rows after 501(c)(3) filtering: {len(df):,}")

    total_after = len(df)
    distinct_states = df["state"].nunique(dropna=True)
    distinct_majors = df["ntee_major"].nunique(dropna=True)
    print(f"Finished cleaning. Rows before: {total_before:,} -> after: {total_after:,}")
    print(f"Distinct states: {distinct_states}, distinct NTEE majors: {distinct_majors}")

    required_cols = ["ein", "name", "city", "state", "ntee_code", "ntee_major"]
    for col in required_cols:
        if col not in df.columns:
            df[col] = pd.NA
    return df[required_cols]

def main() -> None:
    raw_df = load_raw_irs_data(REGION_FILES)
    normalized_df = normalize_columns(raw_df)
    cleaned_df = clean_irs_dataframe(normalized_df)

    OUTPUT_FILE.parent.mkdir(parents=True, exist_ok=True)
    cleaned_df.to_csv(OUTPUT_FILE, index=False, encoding="utf-8")

    print(f"Saved cleaned data to {OUTPUT_FILE} (rows: {len(cleaned_df):,})")
    print("Sample rows:")
    print(cleaned_df.head())

if __name__ == "__main__":
    main()



